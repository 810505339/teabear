<template>
	<view class="main">
		<image :src="`/static/img/page4/${item}.png`" :class="item" v-for="(item, index) in imageList"
			:style="{display:item=='rule_model'&&!show?'none':'block'}" :key="index"
			@click="item=='rule'||item=='rule_model'?showModel():closeModel(item)" :animation="animationData[item]" />
	</view>
</template>
<script setup lang="ts">
	...
	let timer1: any = null,timer2: any = null;
	onMounted(async () => {
